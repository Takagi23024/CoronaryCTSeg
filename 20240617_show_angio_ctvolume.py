...
def rotate_camera_around_volume_center(camera, volume_center, horizontal_angle, vertical_angle):
    ...
    # 水平回転行列を生成 (Z軸周りの回転)
    horizontal_axis = np.array([0, 0, 1])
...
def filter_slice_outside_radius(slice_data, center, radius):
    """
    指定された半径よりも外側のスカラー値を0にするフィルター関数

    Parameters:
    slice_data (np.ndarray): スライスデータ
    center (tuple): 中心点 (x, y)
    radius (float): 半径

    Returns:
    np.ndarray: フィルター適用後のスライスデータ
    """
...
def apply_filter_to_volume(volume_data, center, radius):
    filtered_volume = np.copy(volume_data)
    for i in range(filtered_volume.shape[2]):
        filtered_volume[:, :, i] = filter_slice_outside_radius(filtered_volume[:, :, i], center, radius)
    return filtered_volume


def update_vtk_image(volume_data, spacing):
    vtk_image_data = vtk.vtkImageData()
    vtk_image_data.SetDimensions(volume_data.shape)
    vtk_image_data.SetSpacing(spacing)
    flat_numpy_array = volume_data.flatten(order="F")
    vtk_array = numpy_support.numpy_to_vtk(num_array=flat_numpy_array, deep=True, array_type=vtk.VTK_FLOAT)
    vtk_image_data.GetPointData().SetScalars(vtk_array)
    return vtk_image_data
...
# 半径と中心を定義
radius = 170  # 任意の半径（ピクセル単位）
center = (ArrayDicom.shape[1] // 2, ArrayDicom.shape[0] // 2)  # スライスの中心

# フィルター適用
filtered_ArrayDicom = apply_filter_to_volume(ArrayDicom, center, radius)

# フィルター適用後のデータを使用してvtk_imageを更新
ct_vtk_image = ct_numpy_to_vtk_image(filtered_ArrayDicom, ConstPixelSpacing)
...
def update_slice_text(actor, slice_index, total_slices, rao_lao_angles, cra_cau_angles):
    rao_lao = (
        f"RAO: {-rao_lao_angles[slice_index]:.2f}"
        if rao_lao_angles[slice_index] < 0
        else f"LAO: {rao_lao_angles[slice_index]:.2f}"
    )
    cra_cau = (
        f"CRA: {cra_cau_angles[slice_index]:.2f}"
        if cra_cau_angles[slice_index] > 0
        else f"CAU: {-cra_cau_angles[slice_index]:.2f}"
    )
...
def update_radius_and_render(radius):
    global filtered_ArrayDicom, ct_vtk_image, ct_volume, ct_renderer
    filtered_ArrayDicom = apply_filter_to_volume(ArrayDicom, center, radius)
    ct_vtk_image = update_vtk_image(filtered_ArrayDicom, ConstPixelSpacing)
    ct_volume = setup_volume_rendering(ct_vtk_image)
    ct_renderer.RemoveAllViewProps()
    ct_renderer.AddVolume(ct_volume)
    render_window.Render()


def RadiusChangeCallback(obj, event):
    global radius
    key = obj.GetKeySym()
    if key == "Right":
        radius += 10
        ...
        radius -= 10
    radius = max(0, radius)
    update_radius_and_render(radius)


render_window_interactor.AddObserver("KeyPressEvent", RadiusChangeCallback)
