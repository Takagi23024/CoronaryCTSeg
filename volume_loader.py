...
def load_dicom_and_extract_points(dicom_dir):
    ...
    # DICOMParserが最後に処理した画像の最初のピクセル（左上隅）の（DICOM）x,y,z座標を取得する。
...
def point_cloud_to_scalar_array(point_cloud, pixel_dims, pixel_spacing):
...
def scalar_array_to_vtk_image(scalar_array, pixel_spacing):
    ...
    vtk_array = numpy_to_vtk(flat_array_dicom, deep=True, array_type=4)
...
def create_volume_actor(vtk_image_data):
...
def render_volume(volume, view_orientation="CORONAL"):
...
vtk_image_data, scalar_array, point_cloud, pixel_dims, pixel_spacing, last_image_first_pixel_coords = (
    load_dicom_and_extract_points(dicom_dir)
)

volume = create_volume_actor(vtk_image_data)
render_volume(volume, view_orientation="AXIAL")
render_volume(volume, view_orientation="SAGITTAL")
render_volume(volume, view_orientation="CORONAL")
