...
def on_finish_button_pressed(obj, event):
    ...
    for i in range(len(red_points) - 1):
        start_point = np.array(red_points[i])
        end_point = np.array(red_points[i + 1])
        ...
        if np.linalg.norm(rotation_axis) > 0:  # 回転軸がゼロベクトルでない場合のみ回転
            rotation_axis = rotation_axis / np.linalg.norm(rotation_axis)
        rotation_angle = np.degrees(np.arccos(np.clip(np.dot(y_axis, direction), -1.0, 1.0)))

        # トランスフォームを作成
        transform = vtk.vtkTransform()
        transform.Translate(start_point)  # 底面の中心を始点に設定
        if np.linalg.norm(rotation_axis) > 0:  # 回転軸がゼロベクトルでない場合のみ回転
            transform.RotateWXYZ(rotation_angle, rotation_axis)
        transform.Translate(0, height / 2.0, 0)  # 円柱の中心を中間点に移動
    ...
    extracted_data = append_filter.GetOutput()

    # 抽出されたデータを描画
    extracted_mapper = vtk.vtkPolyDataMapper()
    extracted_mapper.SetInputData(extracted_data)

    extracted_actor = vtk.vtkActor()
    extracted_actor.SetMapper(extracted_mapper)
    extracted_actor.GetProperty().SetColor(0, 1, 0)  # 緑色

    renderer.AddActor(extracted_actor)
    render_window.Render()
